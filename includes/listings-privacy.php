...
function aomark_listings_add_privacy_policy_content() {
	...
	$content .= '<p>' . wp_kses_post( __( '<strong>Map tiles:</strong> Hidden Location tabs in the listing editor do not request map tiles. Opening a Location tab automatically loads its map, so the authorized editor’s browser sends normal requests to the selected tile provider. On the frontend, a visitor’s browser requests map tiles when a page displays a map configured by the site owner. The provider can receive normal web request information such as the person’s IP address, user agent, requested tile coordinates, and referrer according to its policy.', 'aomark-listings' ) ) . '</p>';
