...
function aomark_listings_add_privacy_policy_content() {
	...
	$content .= '<p>' . wp_kses_post( __( '<strong>Address search:</strong> By default, address text entered by an authorized editor is sent from this site’s server to the Photon service operated by Komoot. The search text and normal server request information are processed under the provider’s policies. Responses may be cached temporarily by WordPress to reduce repeated requests.', 'aomark-listings' ) ) . '</p>';
	$content .= '<p>' . wp_kses_post( __( '<strong>Map tiles:</strong> When a page displays a map, the visitor’s browser requests map tiles from OpenStreetMap. The tile provider can receive normal web request information such as the visitor’s IP address, user agent, requested tile coordinates, and referrer according to its policy.', 'aomark-listings' ) ) . '</p>';
