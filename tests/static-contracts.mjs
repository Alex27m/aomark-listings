...
assert.doesNotMatch(models, /data-aomark-location-load-map/, 'Admin maps must not retain the redundant click-to-load control');
assert.match(models, /aomark-listings-location-map-notice/, 'Admin maps must disclose that opening Location loads external tiles');
assert.match(adminFrontend, /editor\.data\('aomark-location-load-map', loadMap\)/, 'Each location editor must retain an idempotent map loader');
assert.match(adminFrontend, /editors\.filter\(':visible'\)/, 'Hidden Location panels must not load map tiles');
assert.match(adminFrontend, /function refreshVisibleLocationMaps[\s\S]*initLocationEditors\(root\);[\s\S]*loadVisibleLocationMaps\(root\);/, 'Every visibility refresh must initialize late-mounted location editors before loading maps');
assert.match(adminFrontend, /refreshVisibleLocationMaps\(panel\);/, 'The activated Location panel must load and resize only its own visible map');
assert.match(adminFrontend, /postbox-toggled[\s\S]*refreshVisibleLocationMaps\(postbox \|\| document\)/, 'Opening a collapsed WordPress postbox must initialize its newly visible map');
assert.match(adminFrontend, /new window\.MutationObserver[\s\S]*mutation\.addedNodes[\s\S]*refreshVisibleLocationMaps\(this\)[\s\S]*observer\.observe\(document\.body, \{ childList: true, subtree: true \}\)/, 'Late-mounted block-editor metaboxes must initialize only when they add a location editor');
assert.match(adminFrontend, /initLocationEditors\(document\);[\s\S]*loadVisibleLocationMaps\(document\);/, 'An initially visible Location-only editor must load its map');
...
assert.match(adminFrontend, /function syncMediaClear[\s\S]*prop\('hidden', !hasValue\)/, 'Media Clear actions must track whether a saved selection exists');
assert.match(adminFrontend, /chooseButton\.trigger\('focus'\);[\s\S]*syncMediaClear\(wrap\);/, 'Clearing media must return focus to Choose before hiding Clear');
...
assert.match(models, /data-aomark-location-map role="region" aria-label=/, 'Admin map canvases need a named semantic region');
